<x-app-layout>
        ...
        <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
            ...
            <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
                <li class="relative before:content-['\ea54'] before:font-remix ltr:before:-right-1 rtl:before:-left-1  before:absolute before:text-[18px] before:-top-[3px] ltr:pr-4 rtl:pl-4 before:text-slate-400 dark:text-zink-200">
                    <a href="#!" class="text-slate-400 dark:text-zink-200">Leaves</a>
                </li>
            </ul>
        </div>
                ...
                <div class="grid items-center grid-cols-12">
                    <div class="col-span-12 lg:col-span-8 2xl:col-span-7">
                        <h5 class="mb-3 font-normal tracking-wide text-slate-200">
                            ...
                            <p>Taking a leave? </p> <br> 
                            <p>Aww! We'll miss you!</p>
                        </h5> 
                        <form method="GET" action="{{ route('leaves.create') }}"> 
                            <button type="submit" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-500/20 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-500/20 dark:ring-custom-400/20">Create Leave Request</button>
                        </form> 
                    </div>
    ...
    <div class="col-span-12">
        <div class="card">
            <div class="card-body">
                <h6 class="mb-4 text-15">Your Leave Requests</h6>
                                ...
                                <tr class="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-4 py-2">
                                        <div class="flex items-center">
                                            <a href="{{ route('leaves.show', $leaveRequest->id) }}" class="text-blue-500 hover:text-blue-600 mr-2">View</a>

                                            @if ($leaveRequest->status === 'pending')
                                                <a href="{{ route('leaves.edit', $leaveRequest->id) }}" class="text-yellow-600 hover:text-yellow-800 mr-2">Edit</a>
                                                <form action="{{ route('leaves.destroy', $leaveRequest->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this leave request?');">
                                                    @csrf
                                                    @method('DELETE')
