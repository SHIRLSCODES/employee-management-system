<x-app-layout>
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
            <div class="grow">
                <h5 class="text-16">My Stock Requisitions</h5>
            </div>
            <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
                ...
                <li class="text-slate-700 dark:text-zink-100">Stock Requisitions</li>
            </ul>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
                    ...
                    <a href="{{ route('stockRequisitions.create') }}" 
                       class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                        ...
                        <span class="align-middle">New Requisition</span>
                    </a>
                </div>
                        ...
                        <table class="w-full whitespace-nowrap">
                            <thead class="ltr:text-left rtl:text-right bg-slate-100 text-slate-500 dark:bg-zink-600 dark:text-zink-200">
                                <tr>
                                    ...
                                    <th class="px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500">Requested</th>
                            ...
                            <tbody>
                                @foreach($requisitions as $requisition)
                                    <tr class="border-b border-slate-200 dark:border-zink-500">
                                        <td class="px-3.5 py-2.5">#{{ $requisition->id }}</td>
                                        <td class="px-3.5 py-2.5">
                                            <div class="flex flex-col">
                                                <span class="font-medium text-slate-600 dark:text-zink-200">{{ $requisition->stockItem->name }}</span>
                                                @if($requisition->stockItem->description)
                                                    <span class="text-xs text-slate-500 dark:text-zink-400">{{ Str::limit($requisition->stockItem->description, 40) }}</span>
                                                @endif
                                            </div>
                                        </td>
                                        <td class="px-3.5 py-2.5">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800 dark:bg-zink-600 dark:text-zink-200">
                                                {{ $requisition->stockItem->category }}
                                            </span>
                                        </td>
                                        <td class="px-3.5 py-2.5">
                                            <span class="font-medium">{{ number_format($requisition->quantity) }}</span>
                                            <span class="text-slate-500 dark:text-zink-400">{{ $requisition->stockItem->unit }}</span>
                                        </td>
                                        <td class="px-3.5 py-2.5">
                                        ...
                                        <td class="px-3.5 py-2.5">
                                            <div class="flex flex-col">
                                                <span class="text-sm">{{ $requisition->created_at->format('M d, Y') }}</span>
                                                <span class="text-xs text-slate-500 dark:text-zink-400">{{ $requisition->created_at->format('H:i') }}</span>
                                            </div>
                                        </td>
                                        <td class="px-3.5 py-2.5">
                                            <div class="flex gap-2">
                                                <a href="{{ route('stockRequisitions.show', $requisition) }}" 
                                                ...
                                                @if($requisition->status === 'pending')
                                                    <a href="{{ route('stockRequisitions.edit', $requisition) }}" 
                                                       class="flex items-center justify-center size-[30px] p-0 text-slate-500 btn bg-slate-100 hover:text-white hover:bg-slate-600 focus:text-white focus:bg-slate-600 focus:ring focus:ring-slate-100 active:text-white active:bg-slate-600 active:ring active:ring-slate-100 dark:bg-slate-500/20 dark:text-slate-400 dark:hover:bg-slate-500 dark:hover:text-white dark:focus:bg-slate-500 dark:focus:text-white dark:active:bg-slate-500 dark:active:text-white dark:ring-slate-400/20"
                                                    ...
                                                    <form action="{{ route('stockRequisitions.destroy', $requisition) }}" 
                                                          method="POST" 
                                                          class="inline-block"
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <h5 class="mb-2 text-slate-500 dark:text-zink-200">No Stock Requisitions Found</h5>
                        <p class="mb-6 text-slate-400 dark:text-zink-300">You haven't made any stock requisitions yet.</p>
                        <a href="{{ route('stockRequisitions.create') }}" 
                           class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-lucide="plus" class="lucide lucide-plus inline-block mr-1 size-4">
                                <path d="M5 12h14"></path>
                            ...
                            <span class="align-middle">Create Your First Requisition</span>
                        </a>
                    </div>
                @endif
            </div>
