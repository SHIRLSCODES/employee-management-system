        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.departments.store') }}" method="POST">
                    @csrf
                    <h6 class="mb-4 text-gray-800 text-16 dark:text-zink-50">Add New Department</h6>
                    <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                        <div class="xl:col-span-6">
                            <label for="name" class="inline-block mb-2 text-base font-medium">Name:</label>
                            <x-text-input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="E.g: Human Resource" required/>
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>
                        <div class="xl:col-span-6">
                            <label for="code" class="inline-block mb-2 text-base font-medium">Short Name:</label>
                            <x-text-input type="text" id="code" name="code" value="{{ old('code') }}" placeholder="E.g: HR" required/>
                            <x-input-error :messages="$errors->get('code')" class="mt-2" />
                        </div><!--end col-->
                    </div>
                    <div>
                        <label for="description" class="inline-block mb-2 text-base font-medium">Description:</label>
                        <x-text-area id="description" name="description">{{ old('description') }}</x-text-area>
