<x-admin-layout>
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        ...
        <div class="card">
            <div class="card-body">
                <h6 class="mb-4 text-15">All Admins</h6>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full" id="admin-table">
                        ...
                        <tbody id="admin-table-body">
                            @foreach ($admins as $admin)
                                <tr class="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-4 py-2">{{ $admin->department->name ?? 'N/A' }}</td>
                                    <td class="px-4 py-2">
                                        <div class="flex items-center">
                                            <a href="{{ route('admin.admin.show', $admin->id) }}" class="text-blue-500 hover:text-blue-600 mr-2">View</a>
                                            <a href="{{ route('admin.admin.edit', $admin->id) }}" class="text-yellow-600 hover:text-yellow-800 mr-2">Edit</a>
                                            <form action="{{ route('admin.admin.delete', $admin->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this admin?');">
                                                @csrf
                                                @method('DELETE')
                    ...
                    <x-pagination-tailwind :items="$admins" />
                </div>
            </div>
        </div>
    </div>
        ...
        function searchAdmins() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    $('#admin-table-body').html(data.html);
                }
            });
        }

        $('#admin-search').on('keyup', debounce(searchAdmins, 300));
    </script>
</x-admin-layout>
