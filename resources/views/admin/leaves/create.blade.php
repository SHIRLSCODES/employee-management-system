<x-admin-layout>
    <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
        ...
        <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
            <div class="grow">
                <h5 class="text-16">Create a Leave Request</h5>
            </div>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.leaves.store') }}" method="POST">
                    @csrf
                    <h6 class="mb-4 text-gray-800 text-16 dark:text-zink-50">Create a new leave request</h6>
                    <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                        ...
                        <div class="xl:col-span-3">
                            ...
                            <x-text-input type="date" id="start_date" name="start_date" value="{{ old('start_date') }}" required />
                            <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
                        </div>

                        <div class="xl:col-span-3">
                            <label for="end_date" class="inline-block mb-2 text-base font-medium">End Date:</label>
                            <x-text-input type="date" id="end_date" name="end_date" value="{{ old('end_date') }}" required />
                            <x-input-error :messages="$errors->get('end_date')" class="mt-2" />
                        </div>

                        <div class="xl:col-span-3">
                            <label for="number_of_days" class="inline-block mb-2 text-base font-medium">Number of Days:</label>
                            <x-text-input type="number" id="number_of_days" name="number_of_days" value="{{ old('number_of_days') }}" required min="1" />
